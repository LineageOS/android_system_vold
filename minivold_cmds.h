...
#ifndef _MINIVOLD_CMDS_H
#define _MINIVOLD_CMDS_H
...
static const struct minivold_cmd minivold_cmds[] = {
    { "e2fsck",         e2fsck_main },
    { "mke2fs",         mke2fs_main },
...
struct minivold_cmd get_command(char* command) {
    int i;

    for (i = 0; minivold_cmds[i].name; i++) {
        if (strcmp(command, minivold_cmds[i].name) == 0)
            break;
    }

    return minivold_cmds[i];
}
#endif
